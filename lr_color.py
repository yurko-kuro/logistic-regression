...
class CatImageClassifier:
    def __init__(self, image_path=None):
        ...
        # Configure logging
        logging.basicConfig(
            filename='model_training_color.log',  # Log file name
            level=logging.INFO,  # Logging level
            format='%(asctime)s - %(levelname)s - %(message)s'  # Log format
        )

    def load_dataset(self):
        """Load dataset with color images."""
        logging.info("Loading the dataset with color images.")
        ...
        # No conversion to grayscale here, we use the images as they are
        ...
        logging.info("Dataset loaded with color images.")
    ...
    def preprocess_data(self):
        ...
        train_set_x, train_set_y, test_set_x, test_set_y, classes = self.load_dataset()

        self.num_px = train_set_x[0].shape[0]
    ...
    def save_model(self):
        ...
        np.savez('model_weights_color.npz', w=self.w, b=self.b)
    ...
    def predict_image(self, image_path):
        ...
        img_resized = cv2.resize(img, (self.num_px, self.num_px)).reshape((1, self.num_px * self.num_px * 3)).T
        img_resized = img_resized / 255.0

        image_predict = self.predict(self.w, self.b, img_resized)
